  ...
  return (
    <div className="flex flex-col w-[250px] p-4 bg-white/5   backdrop-blur-sm   animate-slideup rounded-lg cursor-pointer">
      <div className="relative w-full h-56 group">
        ...
        <img src={song.attributes.artwork.url} alt="images_song" />
      </div>

      <div className="mt-4 flex flex-col">
        <p className="font-semibold text-lg text-white truncate">
          <Link to={`/song/${song?.id}`}>{song.attributes.name}</Link>
        </p>
        <p className="text-sm truncate text-gray-300 mt-1">
          <Link to={song.attributes.artisName ? `artists/${song?.attributes.artistName}` : '/top-artists'}>{song.attributes.artistName}</Link>
        </p>
      </div>
    </div>
  );
